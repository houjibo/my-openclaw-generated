...
import { IntentNode, IntentEdge, IntentPath, RelationshipType } from './types';

/**
 * 意图网络类
 * 管理意图节点和意图之间的关系
 */
export class IntentNetwork {
  ...
  findPrerequisites(intentId: string): Intent[] {
    const edges = this.findRelationships(
      intentId,
   ...
   * 查找下一步意图
   */
  findNextSteps(intentId: string): Array<{ intent: Intent; weight: number }> {
    const edges = this.findRelationships(
      intentId,
   ...
   * 查找替代方案
   */
  findAlternatives(intentId: string): Array<{ intent: Intent; weight: number }> {
    const edges = this.findRelationships(
      intentId,
   ...
   * 查找相关意图
   */
  findRelatedIntents(intentId: string, maxResults: number = 10): Array<{ intent: Intent; weight: number }> {
    const edges = this.findRelationships(
      intentId,
  ...
  findShortestPath(fromId: string, toId: string): IntentPath | null {
    ...
    const queue: Array<{ id: string; path: string[]; totalWeight: number }> = [
   ...
   * 查找所有路径（DFS）
   */
  findAllPaths(fromId: string, toId: string, maxDepth: number = 10): IntentPath[] {
    if (!this.nodes.has(fromId) || !this.nodes.has(toId)) {
      return [];
    }
    ...
    const dfs = (currentId: string, path: string[], weight: number) => {
      if (currentId === toId) {
  ...
  recommendIntents(
    ...
    limit: number = 5
  ): Array<{ intent: Intent; score: number; reason: string }> {
    const recommendations: Array<{ intent: Intent; score: number; reason: string }> = [];
    ...
    // 去重并排序
    const unique = new Map<string, typeof recommendations[0]>();
   ...
   * 检测冲突
   */
  detectConflicts(intentId: string): Array<{ intent: Intent; reason: string }> {
    const edges = this.findRelationships(
      intentId,
   ...
   * 导出网络为 JSON
   */
  toJSON(): { intents: Intent[]; edges: IntentEdge[] } {
    return {
   ...
   * 导入网络从 JSON
   */
  static fromJSON(data: { intents: Intent[]; edges: IntentEdge[] }): IntentNetwork {
  ...
  toGraphviz(): string {
    ...
    const edgeStyles: Record<string, string> = {
      [RelationshipType.PREREQUISITE]: '[style=dashed, color=blue]',
      [RelationshipType.NEXT_STEP]: '[style=bold, color=green]',
      [RelationshipType.ALTERNATIVE]: '[style=dotted, color=orange]',
      [RelationshipType.RELATED]: '[style=solid, color=gray]',
      [RelationshipType.CONFLICT]: '[style=bold, color=red]',
      [RelationshipType.REQUIRES]: '[color=purple]'
    };
   ...
   * 构建路径对象
   */
  private buildPath(pathIds: string[], totalWeight: number): IntentPath {
